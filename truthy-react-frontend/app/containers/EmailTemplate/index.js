...
import { makeIsLoadingSelector, makeLimitSelector, makePageNumberSelector } from "containers/EmailTemplate/selectors";
...
const EmailTemplate = () => {
  ...
  const onchangeFormMethod = formMethod => dispatch(setFormMethodAction(formMethod));
  const onKeywordChange = keywords => dispatch(setKeywordsAction(keywords)) && loadTemplates();
  const onCreate = () => {
  ...
  return (
    <>
      ...
      <div className="truthy-table ">
        <EmailTemplateTable onCreate={onCreate} onEdit={onEdit} onDelete={onDelete} />
      </div>

      <CreateEmailTemplateModal visible={createEmailTemplate} onCancel={() => setCreateEmailTemplate(false)} />
      <EditEmailTemplateModal visible={editEmailTemplate} onCancel={() => setEditEmailTemplate(false)} />
    </>
  );
};
