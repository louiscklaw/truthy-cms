...
import { setPageNumberAction, setPageSizeAction } from "containers/EmailTemplate/actions";
...
import { makeIsLoadingSelector, makeTemplatesSelector } from "containers/EmailTemplate/selectors";
...
function EmailTemplateTable(props) {
  ...
  const paginationOptions = {
    ...
    showTotal: (total, range) => (
      <FormattedMessage {...commonMessages.pagination} values={{ start: range[0], end: range[1], total }} />
    ),
  };
  ...
  return (
    <>
      <Table
        ...
        scroll={{ x: 500 }}
      >
        <Table.Column title={intl.formatMessage(messages.titleLabel)} dataIndex="title" key="title" width={100} />
        <Table.Column title={intl.formatMessage(messages.subjectLabel)} dataIndex="subject" key="subject" width={100} />
        <Table.Column title={intl.formatMessage(messages.senderLabel)} dataIndex="sender" key="sender" width={100} />
        <Table.Column
          ...
          render={(_, { id }) => (
            <>
              {checkPermissionForComponent(user.role, EditRoutePermission) ? (
                <ToolTipButtonWrapper title={commonMessages.editLabel} clickEvent={() => onEdit(id)}>
                  <EditOutlined />
