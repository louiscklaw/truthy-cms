...
import { queryUserStatsAction, queryDeviceAction, setDeviceTypeAction } from "containers/Dashboard/actions";
...
export default function Dashboard() {
  ...
  const { userStats, isLoading, deviceChart, deviceType } = useSelector(stateSelector);
  const loadUserStats = () => dispatch(queryUserStatsAction());
  const loadDeviceStats = () => dispatch(queryDeviceAction());
  const handleDeviceChange = e => dispatch(setDeviceTypeAction(e.target.value));
