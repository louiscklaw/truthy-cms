...
import { enterRegisterAction, setFormValuesAction, clearFormAction } from "containers/RegisterPage/actions";
import {
  makeErrorSelector,
...
function RegisterForm(props) {
  ...
  const { errors, isLoading, initialValues, clearFormValue } = useSelector(stateSelector);
  const [form] = Form.useForm();
  const [password, setPassword] = useState("");

  const [lowerCheck, upperCheck, numChecker, charCheck] = usePasswordStrengthCheckHook(password);
  ...
  const checkConfirm = (rule, value) => {
    ...
    if (newPassword !== value) {
      return Promise.reject(new Error(intl.formatMessage(commonMessage.confirmPasswordMatchError)));
    }
  ...
  return (
    ...
    >
      ...
      >
        <Progress percent={((lowerCheck + charCheck + upperCheck + numChecker) / 4) * 100} steps={4} />
      </FormInputWrapper>
        ...
        rules={[
          {
            required: true,
            whitespace: true,
            message: <FormattedMessage {...commonMessage.confirmPasswordRequired} />,
          },
          {
        ...
        rules={[
          {
            validator: (_, val, callback) =>
              val ? callback() : callback(<FormattedMessage {...messages.acceptTerm} />),
          },
        ]}
        name="accept"
        id="accept"
        ...
        <FormattedMessage
          ...
          values={{
            TermsAndConditionsLink: (
              <Link to="/terms-agreement">{intl.formatMessage(messages.termsAndConditions)}</Link>
            ),
          }}
        />
      </FormCheckboxWrapper>
