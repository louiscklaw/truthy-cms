...
import { disableTokenAction, queryRefreshTokenListAction } from "containers/UserAccount/actions";
  ...
  return (
    ...
    >
      <Skeleton title={false} loading={item.loading} active>
        <List.Item.Meta
          title={<FormattedMessage {...messages.osDetail} values={{ os: ua.os.name, version: ua.os.version }} />}
          description={
            <FormattedMessage
...
const LoginActivity = () => {
  ...
  const onLoadMore = () => {
    setRefreshTokenList(refreshTokenList.concat([...new Array(5)].map(() => ({ loading: true }))));
  ...
  return (
    <List
      ...
      loadMore={loadMore}
      renderItem={item => <ListItem item={item} key={item.id} onDelete={disableRefreshToken} intl={intl} />}
    />
  );
};
