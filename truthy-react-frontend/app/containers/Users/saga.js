...
import { DELETE_ITEM_BY_ID, GET_USER_BY_ID, QUERY_ROLES, QUERY_USERS, SUBMIT_FORM } from "containers/Users/constants";
...
export function* handleSubmitForm() {
  ...
  const payload = ApiEndpoint.makeApiPayload(requestUrl, formMethod, formValues);
  try {
    yield call(request, payload);
    yield put(queryUsersAction());
    yield put(clearFormFieldAction());
    yield put(asyncEndAction());
    const message = formMethod === PUT ? commonMessage.updateSuccess : commonMessage.addSuccess;
