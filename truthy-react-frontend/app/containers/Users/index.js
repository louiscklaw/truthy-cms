...
const Users = () => {
  ...
  const { user, pageNumber, pageSize, isLoading, id } = useSelector(stateSelector);

  const loadUsers = () => dispatch(queryUsersAction());
  const loadRoles = () => dispatch(queryRolesListAction());
  const onKeywordChange = keywords => dispatch(setSearchKeywordAction(keywords)) && loadUsers();
  ...
  return (
    <div className="truthy-wrapper">
      ...
      <CreateUserModal visible={createUser} onCancel={() => setCreateUser(false)} />
