...
import { setPageNumberAction, setPageSizeAction } from "containers/Users/actions";
import { Table, Tag } from "antd";
import { createStructuredSelector } from "reselect";
import { makeIsLoadingSelector, makeUsersSelector } from "containers/Users/selectors";
    ...
    <>
      ...
      >
        <Table.Column
          ...
          render={(_, { name }) => name}
        />
        <Table.Column title={intl.formatMessage(commonMessages.emailLabel)} dataIndex="email" key="email" width={100} />
        <Table.Column
        ...
        <Table.Column
          ...
          dataIndex="status"
          key="status"
          render={(_, { status }) => <Tag color={getStatusClass(status)}>{status}</Tag>}
          width={100}
        />
        <Table.Column
          ...
          key="createdAt"
          render={(_, { createdAt }) => (
            <FormattedMessage {...messages.createdAt} values={{ ts: Date.parse(createdAt) }} />
          )}
          width={100}
        />
        <Table.Column
