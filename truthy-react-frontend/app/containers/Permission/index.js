...
import { makeIsLoadingSelector, makeLimitSelector, makePageNumberSelector } from "containers/Permission/selectors";
import {
...
const Permission = () => {
  ...
  const onKeywordChange = keywords => dispatch(setKeywordsAction(keywords)) && loadPermissions();
  const onCreate = () => {
  ...
  return (
    <div className="truthy-wrapper">
      ...
      <div className="truthy-table ">
        <PermissionTable onCreate={onCreate} onEdit={onEdit} onDelete={onDelete} />
      </div>
      <CreatePermissionModal visible={createPermission} onCancel={() => setCreatePermission(false)} />
      <EditPermissionModal visible={editPermission} onCancel={() => setEditPermission(false)} />
    </div>
  );
};
