...
export function* handleSubmitForm() {
  ...
  try {
    ...
    const message = method === PUT ? commonMessage.updateSuccess : commonMessage.addSuccess;
...
export function* handleQueryPermission() {
  ...
  try {
    ...
    if (response.results) {
      ...
      const groupedPermissionByResource = _.groupBy(permissionCheckBoxArray, "resource");
      const treeStructure = [];
      // eslint-disable-next-line no-restricted-syntax
      for (const resourcePermissionTitle of Object.keys(groupedPermissionByResource)) {
        treeStructure.push({
