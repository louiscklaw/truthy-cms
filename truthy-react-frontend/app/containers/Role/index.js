...
export default function Role() {
  ...
  const onKeywordChange = keywords => dispatch(setKeywordsAction(keywords)) && loadRoles();
  ...
  return (
    <div className="truthy-wrapper">
      ...
      <div className="truthy-table ">
        <RoleTable onCreate={onCreate} onEdit={onEdit} onModifyPermission={onModifyPermission} onDelete={onDelete} />
      </div>
      <CreateRoleModal visible={createRole} onCancel={() => setCreateRole(false)} />
      <EditRoleModal visible={editRole} onCancel={() => setEditRole(false)} />
      <ModifyPermissionModal visible={modifyPermission} onCancel={() => setModifyPermission(false)} />
    </div>
  );
}
