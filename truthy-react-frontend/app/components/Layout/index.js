...
const LayoutPage = () => {
  ...
  const { device, collapsed, otpError, otpVerified, otpValue } = useSelector(stateSelector);
  ...
  useEffect(() => {
    window.onresize = () => {
      const deviceType = /(iPhone|iPad|iPod|iOS|Android)/i.test(navigator.userAgent) ? "MOBILE" : "DESKTOP";
    ...
    <Layout className="layout-page mh-100">
      ...
      <Layout>
        ...
        <Content className="layout-page-content">
          ...
          />
          <Suspense fallback={<LoadingIndicator />}>{otpVerified ? <Outlet /> : null}</Suspense>
        </Content>
      </Layout>
    </Layout>
  );
