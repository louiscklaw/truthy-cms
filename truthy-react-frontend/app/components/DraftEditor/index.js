...
import { customChunkRenderer, entityMapper, entityMapperToComponent } from "components/DraftEditor/helper";
...
class Editor extends Component {
  ...
  onEditorStateChange = editor => {
    const editorHTML = draftToHtml(convertToRaw(editor.getCurrentContent()), null, false, entityMapper);
    this.setState({ editorHTML });
  };

  onEditEditorHTML = ({ target: { value: editorHTML } }) => this.setState({ editorHTML });
  ...
  render() {
    ...
    const ShowEditorCode = () => (
      <ToolWrapper role="presentation" className="rdw-option-wrapper" onClick={this.toggleEditorCode}>
        <CodeOutlined />
        {showEditorCode ? (
      ...
      <>
        ...
        {showEditorCode && (
          <>
            ...
            <TextArea rows={4} value={editorHTML} onChange={this.onEditEditorHTML} />
