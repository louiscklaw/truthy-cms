...
export class AuthController {
  ...
  async login(
    ...
    const { user, cookiePayload, accessToken } = await this.authService.login(userLoginDto, refreshTokenPayload);
    response.setHeader('Set-Cookie', cookiePayload);

    const screen_user = { ...user, password: null, salt: null };

    return response.status(200).json({ accessToken, user: screen_user });
  }
