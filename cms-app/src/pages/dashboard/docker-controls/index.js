...
import { OrderDrawer } from '../../../components/dashboard/docker-controls/order-drawer';
import { OrderListTable } from '../../../components/dashboard/docker-controls/order-list-table';
...
const DockerControls = () => {
  ...
  const [drawer, setDrawer] = useState({ isOpen: false, orderId: undefined });
  ...
  const handleOpenDrawer = orderId => {
    setDrawer({ isOpen: true, orderId });
  };

  const handleCloseDrawer = () => {
    setDrawer({ isOpen: false, orderId: undefined });
  };
  ...
  return (
    <>
      <Head>
        <title>Dashboard: Docker Controls | louislabs</title>
      </Head>
      <Box
        component="main"
        ref={rootRef}
        sx={{ backgroundColor: 'background.paper', display: 'flex', flexGrow: 1, overflow: 'hidden' }}
      >
        <OrderListInner open={drawer.isOpen}>
          <Box sx={{ px: 3 }}>
            <Grid container justifyContent="space-between" spacing={3}>
              <Grid item>
                <Typography variant="h4">{t('DOCKER_CONTROLS')}</Typography>
              </Grid>
