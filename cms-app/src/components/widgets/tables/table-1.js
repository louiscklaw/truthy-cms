...
export const Table1 = () => (
  ...
  >
    <Card>
          ...
          <TableBody>
            {orders.map(order => (
              <TableRow hover key={order.id}>
                ...
                <TableCell>
                  <SeverityPill color={labelColors[order.status]}>{order.status}</SeverityPill>
                </TableCell>
                <TableCell align="right">{format(order.createdAt, 'dd MMM, yyyy HH:mm:ss')}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      >
        <Button endIcon={<ChevronRightIcon fontSize="small" />} size="small" sx={{ cursor: 'pointer' }}>
          See All
        </Button>
      </Box>
    </Card>
